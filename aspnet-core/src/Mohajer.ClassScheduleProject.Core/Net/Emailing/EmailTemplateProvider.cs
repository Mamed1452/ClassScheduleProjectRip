...
namespace Mohajer.ClassScheduleProject.Net.Emailing
{
    public class EmailTemplateProvider : IEmailTemplateProvider, ISingletonDependency
    {
        ...
        public string GetDefaultTemplate(int? tenantId)
        {
            ...
            {
                using (var stream = typeof(EmailTemplateProvider).GetAssembly().GetManifestResourceStream("Mohajer.ClassScheduleProject.Net.Emailing.EmailTemplates.default.html"))
                {
