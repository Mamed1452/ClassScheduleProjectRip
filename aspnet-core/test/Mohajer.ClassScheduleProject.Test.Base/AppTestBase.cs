...
using Mohajer.ClassScheduleProject.Test.Base.TestData;
...
namespace Mohajer.ClassScheduleProject.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="ClassScheduleProjectDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(ClassScheduleProjectDbContext context)
            {
        ...
        protected void UsingDbContext(Action<ClassScheduleProjectDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ClassScheduleProjectDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<ClassScheduleProjectDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<ClassScheduleProjectDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ClassScheduleProjectDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ClassScheduleProjectDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ClassScheduleProjectDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ClassScheduleProjectDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<ClassScheduleProjectDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<ClassScheduleProjectDbContext, Task<TResult>> func)
        {
            TResult result;
