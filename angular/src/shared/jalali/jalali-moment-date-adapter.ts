...
export class JalaliMomentDateAdapter extends DateAdapter<moment.Moment> {
  ...
  //baraie neshun dadane doroste date picker yani harchi ke az back-end migiri daghighan hamoon shekli neshun bede
  //too date picker yani dige niaz be neveshtane isoDate() nis baraie tabdile tarikhe miladi be shamsi
  clone(date: moment.Moment): moment.Moment {
    if (this.locale === 'fa-IR') {
      return moment(date).local(this.locale);
    } else {
  ...
  deserialize(value: any): moment.Moment | null {
    ...
    if (typeof value === 'string') {
      ...
      if (this.locale === 'fa-IR') {
        date = moment(value).locale('fa-IR');
      } else {
