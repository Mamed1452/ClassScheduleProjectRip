import { Component, OnInit, Injector, Input, ViewChild, OnDestroy, ChangeDetectorRef } from '@angular/core';
...
export class CustomizableDashboardComponent extends AppComponentBase implements OnInit, OnDestroy {
  ...
  openDialog() {
    this.dialog.open(FilterDateRangePickerComponent, {
      width: '300px',
  ...
  onDateRangeFilterChange(dateRange = []) {
    ...
    this.endDate = new JalaliPipe().transform(dateRange[1], 'YYYY/MM/DD');
  }
  subDateRangeFilter() {
    abp.event.on('app.dashboardFilters.dateRangePicker.onDateChange',
    (dateRange)=>{
